...
status_t AudioPolicyManager::checkAndSetVolume(int stream, int index, audio_io_handle_t output, uint32_t device, int delayMs, bool force)
{
    ...
    // apply volume attenuation to speaker
    if (device == AudioSystem::DEVICE_OUT_SPEAKER) {
        char speakerBuf[PROPERTY_VALUE_MAX];
        property_get("persist.sys.attn-speaker", speakerBuf, "0");
        ALOGV("setStreamVolume() attenuation [%s]", speakerBuf);
    ...
    // apply volume attenuation to headset/headphone
    if (device == AudioSystem::DEVICE_OUT_WIRED_HEADSET ||
        device == AudioSystem::DEVICE_OUT_WIRED_HEADPHONE) {
        char headsetBuf[PROPERTY_VALUE_MAX];
        property_get("persist.sys.attn-headset", headsetBuf, "0");
        ALOGV("setStreamVolume() attenuation [%s]", headsetBuf);
...
#ifdef HAVE_FM_RADIO
    // apply volume attenuation to FM audio (headset/speaker)
    if (stream == AudioSystem::FM) {
        char fmBuf[PROPERTY_VALUE_MAX];
        property_get("persist.sys.attn-fm", fmBuf, "0");
        ALOGV("setStreamVolume() attenuation [%s]", fmBuf);
