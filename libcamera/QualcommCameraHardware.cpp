...
static SensorType sensorTypes[] = {
        { "5mp", 2064, 2304, true, 2048, 1536, 0x000007ff }, //cooper
...
void QualcommCameraHardware::filterPreviewSizes() {
    ...
    for(uint32_t prop = 0; prop < sizeof(boardProperties) / sizeof(board_property); prop++) {
...
void QualcommCameraHardware::findSensorType(){
    ...
    if (ret) {
        for (uint32_t i = 0; i < sizeof(sensorTypes) / sizeof(SensorType); i++) {
    ...
    cam_ctrl_type type, uint16_t length, void *value)
{
    ...
    ALOGV("%s: fd %d, type %d, length %d", __FUNCTION__, mCameraControlFd, type, length);
...
status_t QualcommCameraHardware::setPreviewFrameRate(const CameraParameters& params)
{
    ...
    if(MINIMUM_FPS <= fps && fps <=MAXIMUM_FPS){
        ...
        bool ret = native_set_parm(CAMERA_SET_PARM_FPS, sizeof(fps), (void *)&fps);
